...
--root_dir yolo文件夹（具备  images/train/...  labels/train/... 的结构）
--save_path COCO文件夹 （输出 COCO 结构文件夹  annotations/...  images/... ）
...
default_root_dir = r'D:\DesktopImages\mvYOLO-Det-TP-Aug'
default_save_path = r'D:\DesktopImages\mvCOCO-Det-TP-Aug'
default_random_split = True
default_split_by_file = True
default_split_ratio = (0.8,0.1,0.1)

parser = argparse.ArgumentParser()
parser.add_argument('--root_dir', default=default_root_dir,type=str, help="root path of images and labels, include ./images and ./labels and classes.txt")
parser.add_argument('--save_path', type=str,default=default_save_path, help="if not split the dataset, give a path to a json file")
parser.add_argument('--random_split', action='store_true',default=default_random_split, help="random split the dataset, default ratio is 8:1:1")
parser.add_argument('--split_by_file', action='store_true',default=default_split_by_file, help="define how to split the dataset, include ./train.txt ./val.txt ./test.txt ")
...
def yolo2coco(arg):
    ...
    folder_anno = os.path.join(save_path, 'annotations')
    folder_images = os.path.join(save_path, 'images')
    ...
    indexes = []
    for dir,_,files in os.walk(originImagesDir):
        if len(files):
            cur_dir = dir.split('\\')[-1]
            for f in files:
                indexes.append(osj(cur_dir,f))
    ...
    if arg.random_split or arg.split_by_file:
        ...
        if arg.random_split:
            ...
            train_img, val_img, test_img = train_test_val_split_random(indexes,*default_split_ratio)
    ...
    for k, index in enumerate(tqdm(indexes)):
        ...
        # 读取图像的宽和高
        src_img_path = osj(root_path, 'images',index)
        ...
        # 添加图像的信息
        index = index.split('\\')[-1]
        dst_img_path = osj(folder_images,index)
    ...
    if arg.random_split or arg.split_by_file:
        for phase in ['train','val','test']:
            json_name = os.path.join(folder_anno, '{}.json'.format(phase))
        ...
        json_name = os.path.join(folder_anno, 'samples.json')
        with open(json_name, 'w') as f:
            json.dump(dataset, f)
            print('Save annotation to {}'.format(json_name))
